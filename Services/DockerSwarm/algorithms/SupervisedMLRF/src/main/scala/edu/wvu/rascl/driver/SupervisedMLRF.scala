...
object SupervisedMLRF { 
  def main(args: Array[String]) {
    if (args.length < 7) {
      System.err.println("Must supply valid arguments: [input filename] [output filePath] [output fileName] [numClasses] [numTrees] " +
        "[impurity] [maxDepth] [maxBins] [percent labeled]")
      System.exit(1)
    }
