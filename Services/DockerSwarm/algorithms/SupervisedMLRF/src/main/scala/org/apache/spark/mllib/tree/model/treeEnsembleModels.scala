...
class RandomForestModel @Since("1.2.0") (
  ...
  /* CORRECTION for newer versions of spark */
...
private[tree] sealed class TreeEnsembleModel(
  ...
  /**
   * Calculates the confidence of a predicted class from a single data point
   *  based on (weighted) majority votes. Confidence is measured as the
   *  number of votes received for the majority class.
   *  
   * @param features array representing a single data point
   * @return a tuple with the probability and predicted category from the trained model
   */  
  def predictConf(features: Vector): (Double, Double) = {
    val prob = trees.map(_.predict(features)).sum / trees.length
    val votes = mutable.Map.empty[Int, Double]
    trees.view.zip(treeWeights).foreach { case (tree, weight) =>
      val prediction = tree.predict(features).toInt
      votes(prediction) = votes.getOrElse(prediction, 0.0) + weight
